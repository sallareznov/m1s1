...
open Probleme_sum_to_partition

let usage () =
	Printf.printf "./solve_sum_to_partition <fichier> : lit l'instance du probleme sum puis le reduit en ";
	...
	if ((Array.length Sys.argv) < 2) then
		...
		begin
			...
			affiche_probleme_sum probleme;
	  		let solution = a_une_solution_sum_to_partition probleme
	  		in
